...
import { LayoutDashboard, User, BookOpen, History, Menu, X, LogOut, Sparkles } from "lucide-react";
...
export default function Sidebar() {
  ...
  const menuItems = [
    { title: "Dashboard", href: "/student/dashboard", icon: <LayoutDashboard className="w-5 h-5" /> },
    { title: "Profile", href: "/student/viewProfile", icon: <User className="w-5 h-5" /> },
    { title: "Quizzes", href: "/student/viewQuizzes", icon: <BookOpen className="w-5 h-5" /> },
    { title: "Quiz History", href: "/student/quizHistory", icon: <History className="w-5 h-5" /> },
  ];
    ...
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-slate-950/95 border-b border-slate-900 backdrop-blur-md text-white h-16 p-4 shadow-lg md:hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="font-extrabold tracking-tight bg-gradient-to-r from-indigo-400 to-cyan-300 bg-clip-text text-transparent">Student Panel</span>
        </div>
        <button
      ...
      <aside
        ...
        `}
      >
        <div className="flex flex-col flex-grow py-8 px-5 overflow-y-auto">
          {/* Header logo */}
          ...
          <div className="mt-auto pt-6 border-t border-slate-800/60">
            <button
              ...
              className="w-full bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all duration-200 cursor-pointer hover:scale-[1.02] text-sm"
            >
